...
public class MsgGroupController {
	...
	@RequestMapping(name = "分页查询信息", value = "/msgGroup/pageQuery")
	@ApiInfo(params = {
	...
	public ResponseFrame pageQuery(MsgGroup msgGroup, String orderby) {
		try {
			if(FrameStringUtil.isNotEmpty(orderby)) {
				List<Orderby> orderbys = FrameJsonUtil.toList(orderby, Orderby.class);
				msgGroup.setOrderbys(orderbys);
			}
			ResponseFrame frame = msgGroupService.pageQuery(msgGroup);
			return frame;
		} catch (Exception e) {
			LOGGER.error("处理业务异常: " + e.getMessage(), e);
			return new ResponseFrame(ResponseCode.SERVER_ERROR);
