 ...
 */
@RestController
public class MsgCldController {

	private static final Logger LOGGER = LoggerFactory.getLogger(MsgCldController.class);

	@Autowired
	private MsgCldService msgCldService;
	
	@RequestMapping(name = "获取详细信息", value = "/msgCld/get")
	@ApiInfo(params = {
	...
	public ResponseFrame get(String id) {
		try {
			...
			frame.setBody(msgCldService.get(id));
	...
	@RequestMapping(name = "新增或修改", value = "/msgCld/saveOrUpdate")
	@ApiInfo(params = {
			@ApiParam(name="id", code="id", value=""),
	}, response = {
			@ApiRes(name="响应码[0成功、-1失败]", code="code", clazz=String.class, value="0"),
	...
	public ResponseFrame saveOrUpdate(MsgCld msgCld) {
		try {
			ResponseFrame frame = msgCldService.saveOrUpdate(msgCld);
			return frame;
		} catch (Exception e) {
			LOGGER.error("处理业务异常: " + e.getMessage(), e);
			return new ResponseFrame(ResponseCode.SERVER_ERROR);
	...
	@RequestMapping(name = "分页查询信息", value = "/msgCld/pageQuery")
	@ApiInfo(params = {
	...
	public ResponseFrame pageQuery(MsgCld msgCld, String orderby) {
		try {
			if(FrameStringUtil.isNotEmpty(orderby)) {
				List<Orderby> orderbys = FrameJsonUtil.toList(orderby, Orderby.class);
				msgCld.setOrderbys(orderbys);
			}
			ResponseFrame frame = msgCldService.pageQuery(msgCld);
			return frame;
		} catch (Exception e) {
			LOGGER.error("处理业务异常: " + e.getMessage(), e);
			return new ResponseFrame(ResponseCode.SERVER_ERROR);
	...
	@RequestMapping(name = "根据主键删除", value = "/msgCld/delete")
	@ApiInfo(params = {
			@ApiParam(name="id", code="id", value=""),
	}, response = {
			@ApiRes(name="响应码[0成功、-1失败]", code="code", clazz=String.class, value="0"),
	...
	public ResponseFrame delete(String id) {
		try {
			ResponseFrame frame = msgCldService.delete(id);
			return frame;
		} catch (Exception e) {
			LOGGER.error("处理业务异常: " + e.getMessage(), e);
			return new ResponseFrame(ResponseCode.SERVER_ERROR);
