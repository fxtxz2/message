...
public class FrameTimeUtil {
	 ...
	 * @param endTime	结束时间
	 * @param type		时间的类型[0秒/1分钟/2小时/3天/4月]
	 * @return
	 */
	public static Long getDateDiff(Date beginTime, Date endTime, Integer type) {
		...
		if(type.intValue() != 4) {
			...
			int yearNum = endCalendar.get(Calendar.YEAR) - beginCalendar.get(Calendar.YEAR);
			time = yearNum * 12 + endCalendar.get(Calendar.MONTH) - beginCalendar.get(Calendar.MONTH);
			int dayNum = endCalendar.get(Calendar.DATE) - beginCalendar.get(Calendar.DATE);
			if(dayNum < 0) {
				time --;
			}
		}
	...
	/*public static void main(String[] args) {
		Date beginTime = FrameTimeUtil.parseDate("2015-07-06", FMT_YYYY_MM_DD);
		Date endTime = FrameTimeUtil.parseDate("2016-06-08", FMT_YYYY_MM_DD);
		System.out.println(getDateDiff(beginTime, endTime, 4));
	}*/
}